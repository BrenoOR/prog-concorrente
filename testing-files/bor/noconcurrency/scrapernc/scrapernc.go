...
func Scraper(url string, quotes *[]Quote, authors *[]Author, pokemons *[]Pokemon, visitedPages *[]URL) {
	links := []string{}
	toVisit := false
	...
	c.OnHTML(".quote", func(h *colly.HTMLElement) {
		...
		if q.About != "Not found" {
			Scraper("http://quotes.toscrape.com"+q.About, quotes, authors, pokemons, visitedPages)
		}
			...
			for i, link := range links {
				...
				if !linkVisited && link != "https://scrapeme.live/shop/page/1/" {
					...
					if i-counter < maxPages {
						...
						Scraper(link, quotes, authors, pokemons, visitedPages)
					} else {
